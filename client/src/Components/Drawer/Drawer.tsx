...
const Drawer: React.FC<Partial<DrawerProps & {open:boolean,anchor:"top" | "right" | "bottom" | "left"}>> = (props) => {
    ...
    className={`w-full h-[12rem] ${handleAnchor()} text-black text-center fixed bg-[#F1F1E8] overflow-x-hidden pt-[1rem] lg:pt-[1.5rem] top-[-12rem] left-0`} 
    style={{transitionDuration:"0.5s",transition:"0.5s top"}}  >
        {children}
   </div> : ""}

    {anchor === "right" ? <div id="sidenav-right" 
    className={`w-[15.625rem] h-full ${handleAnchor()} text-black text-center fixed bg-[#F1F1E8] overflow-x-hidden pt-[1rem] lg:pt-[1.5rem] right-[-15.625rem] top-0`} 
    style={{transitionDuration:"0.5s",transition:"0.5s right"}}  >
        {children}
   </div> : ""}

    {anchor === "bottom" ? <div id="sidenav-bottom" 
    className={`w-full h-[12rem] ${handleAnchor()} text-black text-center fixed bg-[#F1F1E8] overflow-x-hidden pt-[1rem] lg:pt-[1.5rem] bottom-[-12rem] left-0`} 
    style={{transitionDuration:"0.5s",transition:"0.5s bottom"}}  >
        {children}
   </div> : ""}

   {anchor === "left" ? <div id="sidenav-left" 
    className={`w-[15.625rem] h-full ${handleAnchor()} text-black text-center fixed bg-[#F1F1E8] overflow-x-hidden pt-[1rem] lg:pt-[1.5rem] left-[-15.625rem] top-0`} 
