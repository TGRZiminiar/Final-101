...
interface CardHomeProps {
    data:Store | SearchResult | BookMark;
    // data:any
}   
...
export const CardHome: React.FC<CardHomeProps> = ({data}) => {
    ...
    <>
        ...
        <div className="w-full h-[25rem]  max-w-[16.875rem] border-[1px] border-gray-50">{/* max-w-[16.875rem] */}
            <div className="w-full h-[25rem] food-card relative overflow-hidden rounded-xl">
                    ...
                    <div className="info-container overflow-hidden">
                        ...
                        <div className={`hide-info grid  mt-2 overflow-hidden gap-1 grid-cols-1 ${!data.category[1] && "justify-center"}`}>
                            <div className="bg-white shadow-md  p-2 hover:bg-slate-100 ">
                                <button className="cursor-pointer" onClick={() => navigate(`/search?search=\"\"&category=${data.category[0]._id}`)}>
                                   <RestaurantOutlinedIcon fontSize="small"/> {data.category[0].name}
                                </button>
                            </div>
                           {data.category[1].name && 
                            <div className="bg-white shadow-md  p-2 hover:bg-slate-100 ">
                            <button className="cursor-pointer" onClick={() => navigate(`/search?search=\"\"&category=${data?.category[1]?._id}`)}>
